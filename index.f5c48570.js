...
const getRandomImg = async ()=>{
    try {
        const res = await fetch(`https://api.unsplash.com/photos/random/?client_id=${"iu201CIY9yJ7shXlQlsn6brCYjjU3iDaNeBAiiCaz0E"}`);
...
canvas.addEventListener("mousemove", (eventObj)=>{
    if (imgIsLoaded) {
        ...
        ctx.drawImage(originImg, 0, 0, originImg.width, originImg.height, -offsetX, -offsetY, originImg.width, originImg.height);
        ctx.restore();
    }
});
fetchPhotoBtn.addEventListener("click", generateImg); // generateImg();

},{}]},["4qavH","8ZNvh"], "8ZNvh", "parcelRequiree0d3")
